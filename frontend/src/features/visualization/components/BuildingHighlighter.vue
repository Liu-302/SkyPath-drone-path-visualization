...
watch(
  ...
  async ([newView, newMesh, newVisible]: [CameraView | null, any | null, boolean]) => {
    disposeHighlight()
    ...
    if (useBackend.value) {
      try {
        const viewpoint = {
          ...
          direction: newView.direction || { x: 0, y: -1, z: 0 },
          up: newView.up || { x: 0, y: 0, z: 1 },
          fov: newView.fov || CAMERA_CONFIG.fov,
          aspect: newView.aspect || CAMERA_CONFIG.aspect,
